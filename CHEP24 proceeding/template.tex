...
\usepackage{url}
\usepackage{listings}
\lstset{language=Python,keywordstyle={\bfseries \color{blue}},showstringspaces=false}
...
\author{\firstname{Saransh} \lastname{Chopra}\inst{1} \and
        \firstname{Jim} \lastname{Pivarski}\inst{1}\fnsep\thanks{\email{pivarski@princeton.edu}}
}
...
\abstract{Vector is a Python library for 2D, 3D, and Lorentz vectors, especially arrays of vectors, to solve common physics problems in a NumPy-like \cite{harris:2020} way. Vector can currently perform numerical computations, and through this paper, we introduce a new symbolic backend that extends vector's utility to theoretical physicists. The numerical backends of vector enable users to create pure Python Object, NumPy arrays, and Awkward arrays \cite{Pivarski:2018} of vectors. The Object and Awkward backends are also implemented in Numba \cite{lam:2015} to leverage JIT-compiled vector calculations, and support for JAX \cite{Bradbury:2018} and Dask \cite{rocklin:2015} is available within the Awkward backend. The new symbolic backend, built on top of SymPy \cite{Meurer:2017} expressions, showcases vector's ability to support far-flung cases and allows SymPy methods and functions to work on vector classes. Moreover, apart from a few software, high energy physics has maintained a strict separation between tools used by theorists and experimentalists, and vector's SymPy backend aims to bridge this gap, providing a unified computational framework for both communities.
}
%
\maketitle
%
...
Consider the example \ref{vector-obj} performing \codeword{deltaR} operation on two Object type 4D Momentum vectors.
...
The exact same operation can be carried out using the SymPy backend with an almost identical syntax. Example \ref{vector-sympy} shows how SymPy \codeword{symbols} can be passed into \codeword{MomentumSymPy} as arguments just like numerical values are passed into \codeword{MomentumObject}. The \codeword{deltaR} operation on the SymPy vector returns a SymPy expression instead of a numerical value. The obtained SymPy expression is compatible with every SymPy method and function; hence, one can substitute (\codeword{subs}) and evaluate (\codeword{evalf}) the resultant expression to validate the theoretical expression.
...
Furthermore, \ref{vector-sympy-extra} depicts how some of the other SymPy functionalities - using \codeword{simplify} to simplify expressions and converting expressions into code for a particular programming language - can be called on the symbolic results.
...
The space-like and negative time-like cases have different sign conventions, which would have led to \codeword{if-else} statements in the code, making it impossible for SymPy to simplify symbolic expressions. Hence, to make SymPy's simplification work, space-like and negative time-like sign conventions are ignored in the shim layer. Given that most high energy physics analyses deal with positive time-like vectors, this caveat does not hinder the ability to use the vector's SymPy backend in theoretical calculations.
